...
<Tabs items={['Snippet', 'Full Code']}>
  <Tab value="Snippet">
    ...
    agent.onUserMessage(({ message }) => {
        ...
        return {
            ...message,
            content: `${message.content} (Reply in Spanish)`
        };
    });
  ...
  <Tab value="Full Code">
    ...
    const fragola = new Fragola({ model: "gpt-6-astra" });
    const agent = fragola.agent({ name: "SearchAgent", instructions: "You are a helpful assistant" });
...
<Tabs items={['Snippet', 'Full Code']}>
  <Tab value="Snippet">
    ...
    agent.onBeforeModelInvocation((payload, context) => {
        ...
        context.setInstructions("Always mention the current date.", "temp-rule");
    });
    ```
  </Tab>
  <Tab value="Full Code">
...
<Tabs items={['Snippet', 'Full Code']}>
  ...
  <Tab value="Full Code">
    ```typescript
    import { createStore } from "@fragola-ai/agent/store";
    import { tool, Fragola } from "@fragola-ai/agent";
