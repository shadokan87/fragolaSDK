...
- `appendMessages(messages, replaceLast)`
- `updateMessages(callback)`

These helpers mutate `context.state.messages` directly through the agent internals. Use them when
you need low-level control over replacement vs append behavior.
