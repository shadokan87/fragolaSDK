...
# raw
Low-level async helper to mutate messages directly. Avoid unless you need precise control; prefer high-level APIs.
- `updateMessages(callback)`

This helper mutates `context.state.messages` directly through the agent internals. Use it when
you need low-level control over the next full message array.

**Examples**
```ts
// 1) Append a synthetic assistant summary
await agent.context.raw.updateMessages(prev => [
  ...prev,
...
agent.onAiMessage((msg, partial, ctx) => {
  if (partial !== null && typeof msg.content === 'string' && msg.content.includes('unsafe')) {
    ctx.stopSync();
  }
  return msg;
});
