...
Fragola core only provides the hook primitive. Reusable hook libraries can live in your application code or in separate packages.
...
const analyticsHook = Hook((agent) => {
  ...
  agent.onAiMessage((message, finishReason, usage, context) => {
    if (finishReason !== null) aiMessages++;
    return message;
  });
