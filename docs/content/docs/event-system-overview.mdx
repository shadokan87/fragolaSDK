...
### 1. Spread with new object return
Return a new object with the spread operator. This is functional and predictable.
```typescript
agent.on("userMessage", ({ message }) => {
    return {
        ...message,
...
### 2. Direct reference mutation
Modify the object's properties directly and return it (or return nothing).
```typescript
agent.on("userMessage", ({ message }) => {
    message.content = `[From Web]: ${message.content}`;
    return message; // returning is optional when mutating
});
```

<Callout title="Info">
  You will see both styles used throughout this documentation because they are completely interchangeable for convenience. But feel free to pick the syntax you prefer!
</Callout>
