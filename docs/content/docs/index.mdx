...
- **Designed for control:** You can stop the agent at any stage of its execution loop from within any event, and every input (user) and output (model) can be mutated at will. This gives you complete authority over the conversation and the agent's state. The syntax is designed to be boilerplate-free—for example, here is how you can easily censor phone numbers before the model ever sees them:
  ...
  agent.on("userMessage", ({ message }) => {
      if (typeof message.content === "string") {
          const numbers = findPhoneNumbersInText(message.content);
          
          // Iterate in reverse to avoid messing up indices
          for (const match of numbers.reverse()) {
              message.content = 
                  message.content.slice(0, match.startsAt) + 
                  "[REDACTED]" + 
                  message.content.slice(match.endsAt);
          }
      }
      return message;
  });
