...
- Bypass the handler entirely by providing an `injectConfig` (useful for dynamic/remote tools).
...
agent.onBeforeToolCall(({ config, toolCall }) => {
    ...
    if (toolCall.name === "remoteTool") {
        return {
            injectConfig: { success: true, data: { status: "handled externally" } }
        };
    }
});
```
