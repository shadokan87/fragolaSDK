...
// ── Message rendering ─────────────────────────────────────────────────────────
function trimMiddle(content: string, maxLength = 200) {
  ...
  return `${content.slice(0, startLength)}${separator}${content.slice(-endLength)}`;
}
...
function _renderMessage(msg: ChatCompletionMessageParam, streaming = false) {
  switch (msg.role) {
    case "user": {
      ...
      console.log(chalk.green("│ ") + chalk.white(content.replace(/\n/g, "\n" + chalk.green("│ ") )));
      // Guardrail rejection warning
      const meta = msg.meta as GuardRailMeta | undefined;
      if (meta && meta.guardrail && meta.guardrail.rejected) {
        const reason = meta.guardrail.reason;
        const warningLines = chalk.yellow.bold("⚠ Message rejected : ") +
          "\n" + chalk.yellow("   " + reason);
    ...
    case "assistant": {
      ...
      if (content) {
        console.log(chalk.blue("│ ") + chalk.white(content.replace(/\n/g, "\n" + chalk.blue("│ ") )) + (streaming ? chalk.cyan("▍") : ""));
      }
      if (toolCalls?.length) {
    ...
    case "tool": {
      const content = typeof msg.content === "string" ? msg.content : JSON.stringify(msg.content) ?? "";
      const displayContent = trimMiddle(content, 200);
      console.log(chalk.magenta.bold("┌─ Tool result"));
      console.log(chalk.magenta("│ ") + chalk.white(displayContent.replace(/\n/g, "\n" + chalk.magenta("│ ") )));
...
export const handleError = (e: any) => {
    console.error("An error occured: ", JSON.stringify(e["error"] ? e["error"] : "Unknown error"))
    process.exit(1);
}